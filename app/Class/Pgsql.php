...
class Pgsql {
    ...
    private function getTableForeign($content)
    {
        ...
        if ($source_label == "" || $target_label == "") {
            ...
            if (array_search("" . $source_label, Pgsql::MANY) + 0 >= 1 && array_search("" . $target_label, Pgsql::MANY) + 0 >= 1) {
                $relation = $this->makeManyToMany($content->source->id, $content->target->id);
            } else {
                if (array_search("" . $source_label, Pgsql::ONE) + 0 >= 1 && array_search("" . $target_label, Pgsql::ONE) + 0 >= 1) {
                    $relation = $this->makeOne($content->source->id, $content->target->id);
                } else {
                    if (array_search("" . $source_label, Pgsql::ONE) + 0 >= 1) {
