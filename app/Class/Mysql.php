...
class Mysql
{
    ...
    private function getTableForeign($content)
    {
        ...
        if ($source_label == "" || $target_label == "") {
            ...
            if (array_search("" . $source_label, Mysql::MANY) + 0 >= 1 && array_search("" . $target_label, Mysql::MANY) + 0 >= 1) {
                $relation = $this->makeManyToMany($content->source->id, $content->target->id);
            } else {
                if (array_search("" . $source_label, Mysql::ONE) + 0 >= 1 && array_search("" . $target_label, Mysql::ONE) + 0 >= 1) {
                    $relation = $this->makeOne($content->source->id, $content->target->id);
                } else {
                    if (array_search("" . $source_label, Mysql::ONE) + 0 >= 1) {
    ...
    private function makeForeign($contentForegin, $foreignId)
    {
        ...
        ADD FOREIGN KEY (' . strtok($foreginId, " ") . ')
    ...
    private function makeOne($idSource, $idTarget)
    {
        ...
        ADD FOREIGN KEY (' . strtok($primaryTarget, " ") . ')
    ...
    private function makeManyToMany($idSource, $idTarget)
    {
        ...
        ADD FOREIGN KEY (' . strtok($primarySourceId, " ") . ') REFERENCES ' . $nameSource . '(' . strtok($primarySource, " ") . ');
        ALTER TABLE ' . $tableName . '
        ADD FOREIGN KEY (' . strtok($primaryTargetId, " ") . ') REFERENCES ' . $nameTarget . '(' . strtok($primaryTarget, " ") . ');' . "\n\n";
        return $relation;
    }
